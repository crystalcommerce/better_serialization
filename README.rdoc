...
Also, for json_serialize, if <code>ActiveRecord::Base.include_root_in_json != true</code> for your app (this is the new rails defalut, btw) you will need to use the :class_name option if it's not inferable by the association name, and will probably want modify that class's to_json to include the id. It'll all 'just work' if +include_root_in_json+ is set to true though.
